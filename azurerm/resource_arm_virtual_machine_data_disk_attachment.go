...
func resourceArmVirtualMachineDataDiskAttachmentCreateUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	name := computeVirtualMachineDataDiskAttachmentName(d, managedDisk)
	...
	// TODO: verify a test case when a VM hits the `max data disks` limit (0 with an A0, 1 with an A1 etc)
...
func computeVirtualMachineDataDiskAttachmentName(d *schema.ResourceData, disk *compute.Disk) string {
